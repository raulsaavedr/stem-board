...
| [configuration.md](configuration.md) | `~/.config/herdr-board/config.toml`: Board-owned themes, top-level and typed `[daemon]` settings, config-defined `[harness.NAME]` adapters (argv placeholders, capability catalogs, `resume`), parse/override precedence, and every environment variable. | are changing the Board palette, tuning the daemon, adding a config-defined harness, or need what an environment variable does. |
